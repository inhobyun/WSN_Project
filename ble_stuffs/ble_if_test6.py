...
print ("\nBlooth BLE Device scan completed... [%d] devices are scanned" % scanned_count)
...
time.sleep(1) # wait 1 seconds for rolling
ret_val = p.readCharacteristic( SCD_STE_RESULT_HND )
while int(ret_val[32]) < 2:
    time.sleep(1) # wait 1 seconds for rolling
    ret_val = p.readCharacteristic( SCD_STE_RESULT_HND )
p.writeCharacteristic( SCD_SET_GEN_CMD_HND, b'\x20' )
print("+--- STE STOP...")
...
#    
hex_string = ''.join(['.' + ch if i % 2 == 0 else ch for i, ch in enumerate(ret_val[0:2].hex())])
print ("+--- Accel mean X raw data is [" + hex_string + "]", end = '')
accel_mean_x = float( int( ret_val[0] ) + 256 * int( ret_val[1] ) ) \
              / 10.0
print ("\t=> [%.1f] g" % (accel_mean_x) )
hex_string = ''.join(['.' + ch if i % 2 == 0 else ch for i, ch in enumerate(ret_val[2:4].hex())])
print ("+--- Accel mean Y raw data is [" + hex_string + "]", end = '')
accel_mean_y = float( int( ret_val[2] ) + 256 * int( ret_val[3] ) ) \
              / 10.0
print ("\t=> [%.1f] g" % (accel_mean_y) )
hex_string = ''.join(['.' + ch if i % 2 == 0 else ch for i, ch in enumerate(ret_val[4:6].hex())])
print ("+--- Accel mean Z raw data is [" + hex_string + "]", end = '')
accel_mean_z = float( int( ret_val[4] ) + 256 * int( ret_val[5] ) ) \
              / 10.0
print ("\t=> [%.1f] g" % (accel_mean_z) )
#
