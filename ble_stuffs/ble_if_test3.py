...
TARGET_UUID = "a602215804" # Manufacturer == BOSCH
TARGET_NAME = "SCD-" # Name == "SCD-123456789012345"
target_dev = None
...
for dev in devices:
    ...
    for (adtype, desc, value) in dev.getScanData():
        # Check BOSCH UUID and NAME contains "SCD-"
        # 255 is manufacturer data (1  is Flags, 9 is Name)
        print("  (AD Type=%d) %s = %s" % (adtype, desc, value))
        if adtype == 255 and TARGET_UUID in value: # desc == manufacturer
            target_found += 1
            print("  +--- found BOSCH device!!") 
        if adtype == 9 and TARGET_NAME in value: # desc == Local Name
            target_found += 1
            print("  +--- found SCD device!!")
        if target_found >= 2:
