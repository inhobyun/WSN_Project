...
target_STE_time = 5.    # STE rolling time in secconds
MAX_STE_TIME    = 60.   # max STE rolling time in seconds
#
# STE configuration
#
...
STE_bits_pattern = 2                    # [    5] ?2 data rate - accelerometer ODR 1600Hz
...
class NotifyDelegate(DefaultDelegate):
    ...
    def handleNotification(self, cHandle, data):
        print("**** Notification Handle:", cHandle, ", Notification: [", data, "]", end='\n', flush = True)
...
print ("\tSTE runinug, rolling ", end='', flush = True)
i = 0
while (time_stop - time_start) < MAX_STE_TIME:
    print( ">" if i==0 else ("\x08>" if i%2==0 else "\x08+"), end='', flush = True)
    i += 1    
    time_stop = time.time()
    if int(ret_val[32]) > 1 and (time_stop - time_start) > target_STE_time:
        STE_result_0 = ret_val
        print( " done! Roll count is [%d]" % int(ret_val[32]) )
        break
    ret_val = p.readCharacteristic( SCD_STE_RESULT_HND )
"""
time.sleep(3.)
...
#############################################
time.sleep(1.)
p.writeCharacteristic( SCD_SET_GEN_CMD_HND, b'\x00\x20' )
#############################################
...
#    
print ("+--- STE Stopped... rolling Time [%.3f], Count [%d]" % (time_stop-time_start, int(STE_result_0[32])))
