...
usage: python wsn_server_ASD.py [port#]
...
@app.route('/m_mon_ASD')
def mon_ASD():
    template = env.get_template('m_mon_ASD.html')
    return template.render()
...
#############################################
# ASD monitoring UI - stop
#
@app.route('/post_monASDstop', methods=['POST'])
def post_monASDstop():
    #data = json.loads(request.data)
    #value = data['value']
    #
